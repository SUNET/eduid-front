...
const validatePersonalData = (values) => {
  ...
  ["given_name", "surname", "display_name", "language"].forEach((pdata) => {
    ...
    else if(pdata==="given_name" && !allowAllalphabets.test(values[pdata])){
      errors[pdata] = "only allow letters";
    }
    else if(pdata==="surname" && !allowAllalphabets.test(values[pdata])){
      errors[pdata] = "only allow letters";
    }
  });
  return errors;
