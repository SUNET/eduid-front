...
export const LOCAL_STORAGE_PERSISTED_COUNT_RESEND_PHONE_CODE =
  "COUNT_PHONE_CODE";
...
export const countFiveMin = (key) => {
  ...
  const elementCountDownTime = document.getElementById(
    `count-down-time-${key}`
  );
  // Resend link button will be disabled
  elementResendLink !== null &&
    elementResendLink.classList.remove("button-active");
  let countDownTime = getLocalStorage(
    key === "email"
  ...
  let timer = setInterval(() => {
    ...
    if (elementCountDownTime !== null) {
      elementCountDownTime.innerHTML =
    ...
    if (period < 0) {
      elementCountDownTime &&
        elementCountDownTime.classList.add("display-none");
      ...
      elementCountDownTime &&
        elementCountDownTime.classList.remove("display-none");
    }
  }, 1000);
};
