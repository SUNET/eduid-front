...
const TryAgainOption = ({ translate, handleReset }) => (
...
const ToHomeOption = ({ translate }) => {
  const toHome = useSelector((state) => state.config.eduid_site_url);
  return (
    <div className="option">
      <label>{translate("runtime_error.generic.label.toHome")}</label>
      <div className="icon-text">
        <button
          className="icon"
          onClick={() => {
        ...
        <p>{translate("runtime_error.generic.p.toHome")}</p>
      </div>
    </div>
  );
};
